...
START_TEST(pop_basic)
{
	...
	ck_assert_int_eq(3, sp_stack_popi(s));
	ck_assert_uint_eq(2, s->size);
	ck_assert_int_eq(2, sp_stack_popi(s));
	ck_assert_uint_eq(1, s->size);
	ck_assert_int_eq(0, sp_stack_pushi(s, 4));
	ck_assert_int_eq(4, sp_stack_popi(s));
	ck_assert_uint_eq(1, s->size);
	ck_assert_int_eq(1, sp_stack_popi(s));
...
START_TEST(pop_string)
{
	...
	ck_assert_ptr_nonnull(str = sp_stack_popstr(s));
...
START_TEST(pop_empty)
{
	...
	ck_assert_ptr_null(sp_stack_popstr(s));
...
{
	struct sp_stack *s;
	ck_assert_ptr_nonnull(s = sp_stack_create(1, 20));
	ck_assert_int_eq(0, sp_stack_popi(s));
	ck_assert_int_eq(0, sp_stack_destroy(s, NULL));
