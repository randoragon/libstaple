...
{
	setup(char*, 10);
	ck_assert_int_eq(0, sp_stack_insertstrn(s, 0, "", 0));
	ck_assert_uint_eq(1, s->size);
	ck_assert_str_eq("", sp_stack_peekstr(s));

	ck_assert_int_eq(0, sp_stack_insertstrn(s, 0, "fourth", 3));
	ck_assert_uint_eq(2, s->size);
	ck_assert_str_eq("fou", sp_stack_peekstr(s));
	ck_assert_str_eq("", sp_stack_getstr(s, 1));

	ck_assert_int_eq(0, sp_stack_insertstrn(s, 1, "third", 5));
	ck_assert_uint_eq(3, s->size);
	ck_assert_str_eq("fou", sp_stack_peekstr(s));
	ck_assert_str_eq("third", sp_stack_getstr(s, 1));
	ck_assert_str_eq("", sp_stack_getstr(s, 2));

	ck_assert_int_eq(0, sp_stack_insertstrn(s, 3, "first", 4));
	ck_assert_uint_eq(4, s->size);
	ck_assert_str_eq("fou", sp_stack_peekstr(s));
	ck_assert_str_eq("third", sp_stack_getstr(s, 1));
	ck_assert_str_eq("", sp_stack_getstr(s, 2));
...
START_TEST(insert_bad_args)
{
	struct data a;
	struct sp_stack *s;
	ck_assert_int_eq(SP_EINVAL, sp_stack_inserti(NULL, 0, 1));

	data_init(&a);
	ck_assert_ptr_nonnull(s = sp_stack_create(sizeof(struct data), 10));
	ck_assert_int_eq(SP_EINVAL, sp_stack_insert(NULL, 0, NULL));
	ck_assert_int_eq(SP_EINVAL, sp_stack_insert(NULL, 0, &a));
	ck_assert_int_eq(SP_EINVAL, sp_stack_insert(s, 0, NULL));
	ck_assert_int_eq(0, sp_stack_destroy(s, data_dtor));
	data_dtor(&a);

	ck_assert_ptr_nonnull(s = sp_stack_create(sizeof(char*), 10));
	ck_assert_int_eq(SP_EINVAL, sp_stack_insertstr(NULL, 0, NULL));
	ck_assert_int_eq(SP_EINVAL, sp_stack_insertstr(NULL, 0, "abc"));
	ck_assert_int_eq(SP_EINVAL, sp_stack_insertstr(s, 0, NULL));
	ck_assert_int_eq(SP_EINVAL, sp_stack_insertstrn(NULL, 0, NULL, 0));
	ck_assert_int_eq(SP_EINVAL, sp_stack_insertstrn(NULL, 0, "abc", 0));
	ck_assert_int_eq(SP_EINVAL, sp_stack_insertstrn(s, 0, NULL, 0));
...
START_TEST(insert_bad_index)
{
	...
	ck_assert_int_eq(SP_EINDEX, sp_stack_inserti(s, 1, 1));
	ck_assert_int_eq(0, sp_stack_pushi(s, 1));
	ck_assert_int_eq(SP_EINDEX, sp_stack_inserti(s, 2, 2));
	ck_assert_int_eq(0, sp_stack_pushi(s, 2));
	ck_assert_int_eq(SP_EINDEX, sp_stack_inserti(s, 3, 3));
	...
	data_init(&c);
	ck_assert_ptr_nonnull(s = sp_stack_create(sizeof(struct data), 10));
	ck_assert_int_eq(SP_EINDEX, sp_stack_insert(s, 1, &a));
	ck_assert_int_eq(0, sp_stack_push(s, &a));
	ck_assert_int_eq(SP_EINDEX, sp_stack_insert(s, 2, &b));
	ck_assert_int_eq(0, sp_stack_push(s, &b));
	ck_assert_int_eq(SP_EINDEX, sp_stack_insert(s, 3, &c));
	ck_assert_int_eq(0, sp_stack_destroy(s, data_dtor));
	data_dtor(&c);

	ck_assert_ptr_nonnull(s = sp_stack_create(sizeof(char*), 10));
	ck_assert_int_eq(SP_EINDEX, sp_stack_insertstr(s, 1, "first"));
	ck_assert_int_eq(SP_EINDEX, sp_stack_insertstrn(s, 1, "first", 3));
	ck_assert_int_eq(0, sp_stack_pushstr(s, "first"));
	ck_assert_int_eq(SP_EINDEX, sp_stack_insertstr(s, 2, "second"));
	ck_assert_int_eq(SP_EINDEX, sp_stack_insertstrn(s, 2, "second", 3));
	ck_assert_int_eq(0, sp_stack_pushstr(s, "second"));
	ck_assert_int_eq(SP_EINDEX, sp_stack_insertstr(s, 3, "third"));
	ck_assert_int_eq(SP_EINDEX, sp_stack_insertstrn(s, 3, "third", 3));
	ck_assert_int_eq(0, sp_stack_destroy(s, sp_free));
}
END_TEST
...
START_TEST(insert_buffer_max_limit)
{
	...
	for (i = 0; i < SIZE_MAX / s->elem_size; i++)
		ck_assert_int_eq(0, sp_stack_inserti(s, 0, i));
	ck_assert_int_eq(SP_ERANGE, sp_stack_inserti(s, 0, 0));
	ck_assert_int_eq(0, sp_stack_destroy(s, NULL));

	data_init(&a);
	ck_assert_ptr_nonnull(s = sp_stack_create(sizeof(struct data), 10));
	for (i = 0; i < SIZE_MAX / s->elem_size; i++)
		ck_assert_int_eq(0, sp_stack_insert(s, 0, &a));
	ck_assert_int_eq(SP_ERANGE, sp_stack_insert(s, 0, &a));
	ck_assert_int_eq(0, sp_stack_destroy(s, NULL));
	data_dtor(&a);

	ck_assert_ptr_nonnull(s = sp_stack_create(sizeof(char*), 10));
	for (i = 0; i < SIZE_MAX / s->elem_size; i++)
		ck_assert_int_eq(0, sp_stack_insertstr(s, 0, "abc"));
	ck_assert_int_eq(SP_ERANGE, sp_stack_insertstr(s, 0, "abc"));
	ck_assert_int_eq(SP_ERANGE, sp_stack_insertstrn(s, 0, "abc", 2));
	ck_assert_int_eq(0, sp_stack_destroy(s, sp_free));
}
END_TEST
