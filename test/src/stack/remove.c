...
START_TEST(remove_object)
{
	...
	ck_assert_int_eq(0, sp_stack_remove(s, 0, &t));
	...
	ck_assert_int_eq(0, sp_stack_remove(s, 1, &t));
	...
	ck_assert_int_eq(0, sp_stack_remove(s, 2, &t));
...
START_TEST(remove_bad_args)
{
	...
	ck_assert_int_eq(SP_EINVAL, sp_stack_remove(NULL, 0, &a));
...
START_TEST(remove_bad_index)
{
	...
	ck_assert_int_eq(SP_EINDEX, sp_stack_remove(s, 0, &a));
	ck_assert_int_eq(0, sp_stack_push(s, &a));
	ck_assert_int_eq(SP_EINDEX, sp_stack_remove(s, 1, &a));
	ck_assert_int_eq(0, sp_stack_push(s, &a));
	ck_assert_int_eq(SP_EINDEX, sp_stack_remove(s, 2, &a));
	ck_assert_int_eq(0, sp_stack_destroy(s, NULL));
	data_dtor(&a);
