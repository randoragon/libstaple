...
{
	struct sp_queue *s;
	ck_assert_ptr_nonnull(s = sp_queue_create(5, 10));
...
{
	ck_assert_ptr_null(sp_queue_create(2, 1 + SIZE_MAX / 2));
	ck_assert_ptr_null(sp_queue_create(SIZE_MAX, SIZE_MAX));
}
END_TEST

START_TEST(create_bad_args)
{
	ck_assert_ptr_null(sp_queue_create(0, 10));
	ck_assert_ptr_null(sp_queue_create(10, 0));
	ck_assert_ptr_null(sp_queue_create(0, 0));
}
END_TEST
