...
START_TEST(foreach_ok)
{
	...
	ck_assert_int_eq(0, sp_queue_foreach(s, data_mutate));
	ck_assert_int_eq(0, sp_queue_foreach(s, data_verify));
	teardown(data_dtor);
}
END_TEST
...
{
	setup(int, 10);
	ck_assert_int_eq(SP_EINVAL, sp_queue_foreach(s, NULL));
	ck_assert_int_eq(SP_EINVAL, sp_queue_foreach(NULL, data_mutate));
	ck_assert_int_eq(SP_EINVAL, sp_queue_foreach(NULL, NULL));
	teardown(NULL);
}
END_TEST
...
START_TEST(foreach_bad_callback)
{
	...
	ck_assert_int_eq(SP_ECALLBK, sp_queue_foreach(s, data_mutate_bad));
	teardown(data_dtor);
}
END_TEST
