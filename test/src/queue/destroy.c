...
{
	ck_assert_int_eq(SP_EINVAL, sp_queue_destroy(NULL, NULL));
	ck_assert_int_eq(SP_EINVAL, sp_queue_destroy(NULL, data_dtor));
}
END_TEST
...
START_TEST(destroy_bad_dtor)
{
	...
	ck_assert_int_eq(0, sp_queue_push(s, &a));
	ck_assert_int_eq(SP_ECALLBK, sp_queue_destroy(s, data_dtor_bad));
	ck_assert_int_eq(0, sp_queue_destroy(s, NULL));
	data_dtor(&a);
}
END_TEST
