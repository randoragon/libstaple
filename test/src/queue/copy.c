...
{
	setup(int, 10, 10);
	ck_assert_int_eq(0, sp_queue_copy(s2, s1, NULL));
...
START_TEST(copy_with_enlargement)
{
	...
	setup(int, 1000, 50);
	for (i = 0; i < 420; i++)
		ck_assert_int_eq(0, sp_queue_pushi(s1, i));
	ck_assert_int_eq(0, sp_queue_copy(s2, s1, NULL));
...
START_TEST(copy_object_empty)
{
	...
	ck_assert_int_eq(0, sp_queue_copy(s2, s1, data_cpy));
	ck_assert_uint_eq(0, s1->size);
	ck_assert_uint_eq(0, s2->size);
	teardown(data_dtor);
}
...
START_TEST(copy_object_basic)
{
	...
	for (i = 0; i < 3; i++) {
		...
		ck_assert_int_eq(0, sp_queue_push(s1, &d));
	}
	ck_assert_int_eq(0, sp_queue_copy(s2, s1, data_cpy));
	ck_assert_uint_eq(3, s1->size);
...
{
	setup(int, 10, 10);
	ck_assert_int_eq(SP_EINVAL, sp_queue_copy(NULL, NULL, NULL));
	ck_assert_int_eq(SP_EINVAL, sp_queue_copy(s2, NULL, NULL));
	ck_assert_int_eq(SP_EINVAL, sp_queue_copy(NULL, s1, NULL));
	ck_assert_int_eq(SP_EINVAL, sp_queue_copy(NULL, NULL, data_cpy));
	ck_assert_int_eq(SP_EINVAL, sp_queue_copy(s2, NULL, data_cpy));
	ck_assert_int_eq(SP_EINVAL, sp_queue_copy(NULL, s1, data_cpy));
	teardown(NULL);
}
END_TEST
...
{
	setup(int, 10, 10);
	ck_assert_int_eq(0, sp_queue_pushi(s1, 21));
	ck_assert_int_eq(SP_ECALLBK, sp_queue_copy(s2, s1, data_cpy_bad));
	teardown(NULL);
}
END_TEST
