...
START_TEST(remove_object)
{
	...
	ck_assert_int_eq(0, sp_queue_remove(s, 2, data_dtor));
	ck_assert_uint_eq(2, s->size);
	ck_assert_int_eq(0, data_cmp(&b, sp_queue_peek(s)));
	ck_assert_int_eq(0, data_cmp(&d, sp_queue_get(s, 1)));
	ck_assert_int_eq(0, sp_queue_remove(s, 1, data_dtor));
	...
	ck_assert_int_eq(0, sp_queue_push(s, &a));
	ck_assert_int_eq(0, sp_queue_remove(s, 0, NULL));
	ck_assert_uint_eq(0, s->size);
	teardown(NULL);
}
END_TEST
...
START_TEST(remove_bad_args)
{
	...
	ck_assert_int_eq(SP_EINVAL, sp_queue_remove(NULL, 0, data_dtor));
...
START_TEST(remove_bad_index)
{
	...
	ck_assert_int_eq(SP_EINDEX, sp_queue_remove(s, 0, NULL));
	ck_assert_int_eq(0, sp_queue_push(s, &a));
	ck_assert_int_eq(SP_EINDEX, sp_queue_remove(s, 1, NULL));
	ck_assert_int_eq(0, sp_queue_push(s, &a));
	ck_assert_int_eq(SP_EINDEX, sp_queue_remove(s, 2, NULL));
	ck_assert_int_eq(0, sp_queue_destroy(s, NULL));
	data_dtor(&a);
...
START_TEST(remove_bad_dtor)
{
	...
	ck_assert_int_eq(0, sp_queue_push(s, &b));
	ck_assert_int_eq(0, sp_queue_push(s, &c));
	ck_assert_int_eq(SP_ECALLBK, sp_queue_remove(s, 0, data_dtor_bad));
	teardown(data_dtor);
}
END_TEST
