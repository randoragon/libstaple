...
Test(stack, create)
{
	...
	cr_assert_neq(NULL, s);
	cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
}
...
Test(stack, destroy)
{
	...
	cr_assert_neq(NULL, s);
	cr_assert_eq(RND_EINVAL, rnd_stack_destroy(NULL, NULL), "%d");
	cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
	s = rnd_stack_create(sizeof(struct data), 1000);
	for (i = 0; i < 1000; i++) {
		struct data d;
		cr_assert_eq(0, data_init(&d), "%d");
		cr_assert_eq(0, rnd_stack_push(s, &d), "%d");
	}
	cr_assert_eq(RND_EHANDLER, rnd_stack_destroy(s, data_dtor_bad), "%d");
	cr_assert_eq(0, rnd_stack_destroy(s, data_dtor), "%d");
}
...
Test(stack, push)
{
	...
	{ /* Generic form */
		...
		cr_assert_neq(NULL, s);
		data_init(&d);
		cr_assert_eq(RND_EINVAL, rnd_stack_push(s, NULL), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_push(NULL, &d), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_push(NULL, NULL), "%d");
		cr_assert_eq(0LU, (unsigned long)s->size, "%lu");
		cr_assert_eq(0, rnd_stack_push(s, &d), "%d");
		cr_assert_eq(1LU, (unsigned long)s->size, "%lu");
		cr_assert_eq(0, rnd_stack_push(s, &d), "%d");
		cr_assert_eq(2LU, (unsigned long)s->size, "%lu");
		cr_assert_eq(0, rnd_stack_clear(s, NULL), "%d");
		for (i = 0; i < SIZE_MAX / sizeof(struct data); i++) {
			struct data a, b;
			cr_assert_eq(0, data_init(&a), "%d");
			cr_assert_eq(0, rnd_stack_push(s, &a), "%d");
			cr_assert_eq(0, rnd_stack_get(s, 0, &b), "%d");
			cr_assert_eq(0, data_cmp(&a, &b), "%d");
		}
		cr_assert_eq(RND_ERANGE, rnd_stack_push(s, &d), "%d");
		cr_assert_eq(0, data_dtor(&d), "%d");
		cr_assert_eq(0, rnd_stack_destroy(s, data_dtor), "%d");
	}
...
#define test(T, F1, F2, V, M) do {                                           \
		...
		cr_assert_neq(NULL, s);                                         \
		cr_assert_eq(RND_EINVAL, F1(NULL, (V)), "%d");              \
		cr_assert_eq(0LU, (unsigned long)s->size, "%lu");           \
		cr_assert_eq(0, F1(s, (V)), "%d");                          \
		cr_assert_eq(1LU, (unsigned long)s->size, "%lu");           \
		cr_assert_eq(0, F1(s, (V)), "%d");                          \
		cr_assert_eq(2LU, (unsigned long)s->size, "%lu");           \
		cr_assert_eq(0, rnd_stack_clear(s, NULL), "%d");            \
		for (i = 0; i < SIZE_MAX / sizeof(T); i++) {                 \
			T a = (V);                                           \
			cr_assert_eq(0, F1(s, a), "%d");                    \
			cr_assert_eq(a, F2(s, 0), M);                       \
		}                                                            \
		cr_assert_eq(RND_ERANGE, F1(s, (V)), "%d");                 \
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");          \
		s = rnd_stack_create(sizeof(T) + 1, 1);                      \
		cr_assert_neq(NULL, s);                                         \
		cr_assert_eq(RND_EILLEGAL, F1(s, (V)), "%d");               \
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");          \
	} while (0)
...
Test(stack, peek)
{
	...
	{ /* Generic form */
		...
		cr_assert_neq(NULL, s);
		cr_assert_eq(RND_EILLEGAL, rnd_stack_peek(s, &b), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_peek(s, NULL), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_peek(NULL, &b), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_peek(NULL, NULL), "%d");
		data_init(&a);
		cr_assert_eq(0, rnd_stack_push(s, &a), "%d");
		cr_assert_eq(0, rnd_stack_peek(s, &b), "%d");
		cr_assert_eq(0, data_cmp(&a, &b), "%d");
		cr_assert_eq(0, rnd_stack_destroy(s, data_dtor), "%d");
	}
...
Test(stack, pop)
{
	...
	{ /* Generic form */
		...
		s = rnd_stack_create(sizeof(struct data), 2);
		cr_assert_neq(NULL, s);
		cr_assert_eq(RND_EILLEGAL, rnd_stack_pop(s, NULL), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_pop(NULL, NULL), "%d");
		data_init(&a);
		cr_assert_eq(0, rnd_stack_push(s, &a), "%d");
		cr_assert_eq(1LU, (unsigned long)s->size, "%lu");
		cr_assert_eq(0, rnd_stack_pop(s, &b), "%d");
		cr_assert_eq(0LU, (unsigned long)s->size, "%lu");
		cr_assert_eq(0, data_cmp(&a, &b), "%d");
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
		cr_assert_eq(0, data_dtor(&b), "%d");
	}
...
Test(stack, clear)
{
	...
	s = rnd_stack_create(sizeof(long double), 1000);
	cr_assert_neq(NULL, s);
	cr_assert_eq(RND_EINVAL, rnd_stack_clear(NULL, NULL), "%d");
	cr_assert_eq(0, rnd_stack_clear(s, NULL), "%d");
	cr_assert_eq(0LU, (unsigned long)s->size, "%lu");
	cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
	s = rnd_stack_create(sizeof(struct data), 1000);
	...
	cr_assert_eq(RND_EHANDLER, rnd_stack_clear(s, data_dtor_bad), "%d");
	cr_assert_eq(0, rnd_stack_clear(s, data_dtor), "%d");
	cr_assert_eq(0LU, (unsigned long)s->size, "%lu");
	cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
}
...
Test(stack, foreach)
{
	...
	s = rnd_stack_create(sizeof(long double), 1000);
	cr_assert_neq(NULL, s);
	cr_assert_eq(RND_EINVAL, rnd_stack_foreach(NULL, data_mutate), "%d");
	cr_assert_eq(RND_EINVAL, rnd_stack_foreach(s, NULL), "%d");
	cr_assert_eq(RND_EINVAL, rnd_stack_foreach(NULL, NULL), "%d");
	cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
	s = rnd_stack_create(sizeof(struct data), 1000);
	for (i = 0; i < 1000; i++) {
		struct data d;
	...
	cr_assert_eq(RND_EHANDLER, rnd_stack_foreach(s, data_mutate_bad), "%d");
	cr_assert_eq(0, rnd_stack_foreach(s, data_mutate), "%d");
	cr_assert_eq(0, rnd_stack_foreach(s, data_verify), "%d");
	cr_assert_eq(0, rnd_stack_destroy(s, data_dtor), "%d");
}
...
Test(stack, copy)
{
	...
	cr_assert_neq(NULL, s);
	cr_assert_eq(RND_EINVAL, rnd_stack_copy(NULL, s, NULL), "%d");
	cr_assert_eq(RND_EINVAL, rnd_stack_copy(p, NULL, NULL), "%d");
	cr_assert_eq(RND_EINVAL, rnd_stack_copy(NULL, NULL, NULL), "%d");
	cr_assert_eq(0, rnd_stack_copy(s, p, NULL), "%d");
	...
	cr_assert_eq(0, rnd_stack_copy(p, s, NULL), "%d");
	cr_assert_eq((unsigned long)p->size, (unsigned long)s->size, "%lu");
	cr_assert_eq((unsigned long)p->elem_size, (unsigned long)s->elem_size, "%lu");
	for (i = 0; i < 1000; i++) {
		cr_assert_eq(0, rnd_stack_pushi(s, FRANGE(INT_MIN, INT_MAX)), "%d");
	}
	cr_assert_eq(0, rnd_stack_copy(p, s, NULL), "%d");
	cr_assert_eq((unsigned long)p->size, (unsigned long)s->size, "%lu");
	cr_assert_eq((unsigned long)p->elem_size, (unsigned long)s->elem_size, "%lu");
	...
	cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
	cr_assert_eq(0, rnd_stack_clear(p, NULL), "%d");
	s = rnd_stack_create(sizeof(struct data), 1000);
	for (i = 0; i < 1000; i++) {
		struct data d;
		cr_assert_eq(0, data_init(&d), "%d");
	...
	cr_assert_eq(RND_EHANDLER, rnd_stack_copy(p, s, data_cpy_bad), "%d");
	cr_assert_eq(0, rnd_stack_copy(p, s, data_cpy), "%d");
	cr_assert_eq((unsigned long)p->size, (unsigned long)s->size, "%lu");
	cr_assert_eq((unsigned long)p->elem_size, (unsigned long)s->elem_size, "%lu");
	for (i = 0; i < 1000; i++) {
		struct data a, b;
		cr_assert_eq(0, rnd_stack_get(s, i, &a), "%d");
		cr_assert_eq(0, rnd_stack_get(p, i, &b), "%d");
		cr_assert_eq(0, data_cmp(&a, &b), "%d");
	}
	cr_assert_eq(0, rnd_stack_destroy(s, data_dtor), "%d");
	cr_assert_eq(0, rnd_stack_destroy(p, data_dtor), "%d");
}
...
Test(stack, insert)
{
	...
	{ /* Generic form */
		...
		s = rnd_stack_create(sizeof(int), 1000);
		cr_assert_neq(NULL, s);
		cr_assert_eq(RND_EINVAL, rnd_stack_insert(s, 0, NULL), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_insert(NULL, 0, &a), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_insert(NULL, 0, NULL), "%d");
		cr_assert_eq(RND_EINDEX, rnd_stack_insert(s, 1, &a), "%d");
		cr_assert_eq(0, rnd_stack_insert(s, 0, &a), "%d");
		cr_assert_eq(1LU, (unsigned long)s->size, "%lu");
		cr_assert_eq(10, rnd_stack_geti(s, 0), "%d");
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
		...
		for (i = 0; i < 1000; i++) {
			...
			cr_assert_eq(0, data_init(d + i), "%d");
			cr_assert_eq(0, rnd_stack_insert(s, idx, d + i), "%d");
			cr_assert_eq((unsigned long)i + 1, (unsigned long)s->size, "%lu");
			cr_assert_eq(0, rnd_stack_get(s, idx, &a), "%d");
			cr_assert_eq(0, data_cmp(&a, d + i), "%d");
		}
		cr_assert_eq(0, rnd_stack_destroy(s, data_dtor), "%d");
		...
		for (i = 0; i < 5; i++) {
			cr_assert_eq(0, data_init(d + i), "%d");
			cr_assert_eq(0, rnd_stack_insert(s, i, d + i), "%d");
		}
		for (i = 0; i < 5; i++) {
			struct data a;
			cr_assert_eq(0, rnd_stack_get(s, i, &a), "%d");
		...
		{
			struct data a;
			cr_assert_eq(0, data_init(d + 5), "%d");
			cr_assert_eq(0, rnd_stack_insert(s, 1, d + 5), "%d");
			cr_assert_eq(0, rnd_stack_peek(s, &a), "%d");
			cr_assert_eq(0, data_cmp(&a, d), "%d");
			cr_assert_eq(0, data_init(d + 6), "%d");
			cr_assert_eq(0, rnd_stack_insert(s, 3, d + 6), "%d");
			cr_assert_eq(0, rnd_stack_peek(s, &a), "%d");
			cr_assert_eq(0, data_cmp(&a, d), "%d");
...
#define test(T, F1, F2, V, M) do {                                                          \
		...
		s = rnd_stack_create(sizeof(T), 1000);                                      \
		cr_assert_neq(NULL, s);                                                        \
		cr_assert_eq(RND_EINVAL, F1(NULL, 0, a), "%d");                            \
		cr_assert_eq(RND_EINDEX, F1(s, 1, a), "%d");                               \
		cr_assert_eq(0, F1(s, 0, a), "%d");                                        \
		cr_assert_eq(1LU, (unsigned long)s->size, "%lu");                          \
		cr_assert_eq(a, F2(s, 0), M);                                              \
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");                         \
                                                                                            \
		...
		for (i = 0; i < 5; i++) {                                                   \
			d[i] = (V);                                                         \
			cr_assert_eq(0, F1(s, i, d[i]), "%d");                             \
		}                                                                           \
		for (i = 0; i < 5; i++) {                                                   \
			cr_assert_eq(d[i], F2(s, i), M);                                   \
		}                                                                           \
		d[5] = (V);                                                                 \
		cr_assert_eq(0, F1(s, 1, d[5]), "%d");                                     \
		cr_assert_eq(d[0], F2(s, 0), M);                                           \
		d[6] = (V);                                                                 \
		cr_assert_eq(0, F1(s, 3, d[6]), "%d");                                     \
		cr_assert_eq(d[0], F2(s, 0), M);                                           \
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");                         \
		s = rnd_stack_create(sizeof(T) + 1, 1000);                                  \
		cr_assert_neq(NULL, s);                                                        \
		cr_assert_eq(RND_EILLEGAL, F1(s, 0, (V)), "%d");                           \
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");                         \
	} while (0)
...
Test(stack, qinsert)
{
	...
	{ /* Generic form */
		...
		s = rnd_stack_create(sizeof(int), 1000);
		cr_assert_neq(NULL, s);
		cr_assert_eq(RND_EINVAL, rnd_stack_qinsert(s, 0, NULL), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_qinsert(NULL, 0, &a), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_qinsert(NULL, 0, NULL), "%d");
		cr_assert_eq(RND_EINDEX, rnd_stack_qinsert(s, 1, &a), "%d");
		cr_assert_eq(0, rnd_stack_qinsert(s, 0, &a), "%d");
		cr_assert_eq(1LU, (unsigned long)s->size, "%lu");
		cr_assert_eq(10, rnd_stack_geti(s, 0), "%d");
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
		...
		for (i = 0; i < 1000; i++) {
			...
			size_t idx = IRANGE(0, i);
			cr_assert_eq(0, data_init(d + i), "%d");
			cr_assert_eq(0, rnd_stack_qinsert(s, idx, d + i), "%d");
			cr_assert_eq((unsigned long)i + 1, (unsigned long)s->size, "%lu");
			cr_assert_eq(0, rnd_stack_get(s, idx, &a), "%d");
			cr_assert_eq(0, data_cmp(&a, d + i), "%d");
		}
		...
		for (i = 0; i < 5; i++) {
			cr_assert_eq(0, data_init(d + i), "%d");
			cr_assert_eq(0, rnd_stack_qinsert(s, i, d + i), "%d");
		}
		for (i = 0; i < 5; i++) {
			struct data a;
			cr_assert_eq(0, rnd_stack_get(s, i, &a), "%d");
			cr_assert_eq(0, data_cmp(&a, d + (8 - i) % 5), "%d");
		}
		{
			struct data a;
			cr_assert_eq(0, data_init(d + 5), "%d");
			cr_assert_eq(0, rnd_stack_qinsert(s, 1, d + 5), "%d");
			cr_assert_eq(0, rnd_stack_peek(s, &a), "%d");
			cr_assert_eq(0, data_cmp(&a, d + 3), "%d");
			cr_assert_eq(0, data_init(d + 6), "%d");
			cr_assert_eq(0, rnd_stack_qinsert(s, 3, d + 6), "%d");
			cr_assert_eq(0, rnd_stack_peek(s, &a), "%d");
			cr_assert_eq(0, data_cmp(&a, d + 2), "%d");
			cr_assert_eq(7LU, (unsigned long)s->size, "%lu");
			cr_assert_eq(0, rnd_stack_destroy(s, data_dtor), "%d");
		}
	}
...
Test(stack, remove)
{
	...
	{ /* Generic form */
		...
		s = rnd_stack_create(sizeof(int), 1000);
		cr_assert_neq(NULL, s);
		cr_assert_eq(RND_EINDEX, rnd_stack_remove(s, 0, &a), "%d");
		cr_assert_eq(0, data_init(&a), "%d");
		cr_assert_eq(0, rnd_stack_push(s, &a), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_remove(NULL, 0, &a), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_remove(NULL, 0, NULL), "%d");
		cr_assert_eq(RND_EINDEX, rnd_stack_remove(s, 1, &a), "%d");
		cr_assert_eq(1LU, (unsigned long)s->size, "%lu");
		cr_assert_eq(0, rnd_stack_remove(s, 0, NULL), "%d");
		cr_assert_eq(0LU, (unsigned long)s->size, "%lu");
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
		cr_assert_eq(0, data_dtor(&a), "%d");

		s = rnd_stack_create(sizeof(struct data), 100);
		cr_assert_neq(NULL, s);
		for (i = 0; i < 100; i++) {
			cr_assert_eq(0, data_init(d + i), "%d");
			cr_assert_eq(0, rnd_stack_push(s, d + i), "%d");
		}
		for (i = 0; i < 100; i++) {
			struct data a;
			size_t idx = IRANGE(0, s->size - 1), j;
			int found = 0;
			cr_assert_eq(0, rnd_stack_remove(s, idx, &a), "%d");
			for (j = 0; j < 100; j++) {
		...
		cr_assert_eq(0, rnd_stack_remove(s, 3, &a), "%d");
		cr_assert_eq(0, data_cmp(d + 4, &a), "%d");
		cr_assert_eq(7LU, (unsigned long)s->size, "%lu");
		cr_assert_eq(0, rnd_stack_remove(s, 0, &a), "%d");
		cr_assert_eq(0, data_cmp(d + 7, &a), "%d");
		cr_assert_eq(6LU, (unsigned long)s->size, "%lu");
		cr_assert_eq(0, rnd_stack_remove(s, 3, &a), "%d");
		cr_assert_eq(0, data_cmp(d + 2, &a), "%d");
		cr_assert_eq(5LU, (unsigned long)s->size, "%lu");
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
		for (i = 0; i < 100; i++) {
			cr_assert_eq(0, data_dtor(d + i), "%d");
		}
	}
...
Test(stack, qremove)
{
	...
	{ /* Generic form */
		...
		s = rnd_stack_create(sizeof(int), 1000);
		cr_assert_neq(NULL, s);
		cr_assert_eq(RND_EINDEX, rnd_stack_qremove(s, 0, &a), "%d");
		cr_assert_eq(0, data_init(&a), "%d");
		cr_assert_eq(0, rnd_stack_push(s, &a), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_qremove(NULL, 0, &a), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_qremove(NULL, 0, NULL), "%d");
		cr_assert_eq(RND_EINDEX, rnd_stack_qremove(s, 1, &a), "%d");
		cr_assert_eq(1LU, (unsigned long)s->size, "%lu");
		cr_assert_eq(0, rnd_stack_qremove(s, 0, NULL), "%d");
		cr_assert_eq(0LU, (unsigned long)s->size, "%lu");
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
		cr_assert_eq(0, data_dtor(&a), "%d");
		...
		for (i = 0; i < 100; i++) {
			...
			size_t idx = IRANGE(0, s->size - 1), j;
			int found = 0;
			cr_assert_eq(0, rnd_stack_qremove(s, idx, &a), "%d");
			for (j = 0; j < 100; j++) {
				/* Lazy and slow way to check, but on average it's
				 * enough */
				if (data_cmp(&a, d + j) == 0) {
		...
		cr_assert_eq(0, rnd_stack_qremove(s, 3, &a), "%d");
		cr_assert_eq(0, data_cmp(d + 4, &a), "%d");
		cr_assert_eq(0, rnd_stack_get(s, 3, &a), "%d");
		cr_assert_eq(0, data_cmp(d + 3, &a), "%d");
		cr_assert_eq(0, rnd_stack_qremove(s, 0, &a), "%d");
		cr_assert_eq(0, data_cmp(d + 6, &a), "%d");
		cr_assert_eq(0, rnd_stack_get(s, 0, &a), "%d");
		cr_assert_eq(0, data_cmp(d + 5, &a), "%d");
		cr_assert_eq(0, rnd_stack_qremove(s, 3, &a), "%d");
		cr_assert_eq(0, data_cmp(d + 2, &a), "%d");
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
		for (i = 0; i < 100; i++) {
...
Test(stack, get)
{
	...
	{ /* Generic form */
		...
		s = rnd_stack_create(sizeof(int), 1000);
		cr_assert_neq(NULL, s);
		cr_assert_eq(RND_EINDEX, rnd_stack_get(s, 0, &a), "%d");
		cr_assert_eq(0, rnd_stack_pushi(s, 10), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_get(NULL, 0, &a), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_get(s, 0, NULL), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_get(NULL, 0, NULL), "%d");
		cr_assert_eq(RND_EINDEX, rnd_stack_get(s, 1, &a), "%d");
		cr_assert_eq(0, rnd_stack_get(s, 0, &a), "%d");
		cr_assert_eq(10, a, "%d");
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
		s = rnd_stack_create(sizeof(struct data), 1000);
		...
		for (i = 0; i < 1000; i++) {
			struct data a;
			cr_assert_eq(0, rnd_stack_get(s, i, &a), "%d");
			cr_assert_eq(0, data_cmp(&a, d + 999 - i), "%d");
		}
		cr_assert_eq(0, rnd_stack_destroy(s, data_dtor), "%d");
	}
...
Test(stack, set)
{
	...
	{ /* Generic form */
		...
		s = rnd_stack_create(sizeof(int), 1000);
		cr_assert_neq(NULL, s);
		cr_assert_eq(RND_EINDEX, rnd_stack_set(s, 0, &a), "%d");
		cr_assert_eq(0, rnd_stack_pushi(s, 10), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_set(NULL, 0, &a), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_set(s, 0, NULL), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_set(NULL, 0, NULL), "%d");
		cr_assert_eq(RND_EINDEX, rnd_stack_set(s, 1, &a), "%d");
		cr_assert_eq(0, rnd_stack_set(s, 0, &a), "%d");
		cr_assert_eq(a, rnd_stack_geti(s, 0), "%d");
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
		s = rnd_stack_create(sizeof(struct data), 1000);
		cr_assert_neq(NULL, s);
		for (i = 0; i < 1000; i++) {
		...
		for (i = 0; i < 1000; i++) {
			struct data a, b;
			cr_assert_eq(0, data_init(&b), "%d");
			cr_assert_eq(0, rnd_stack_get(s, i, &a), "%d");
			cr_assert_eq(0, data_dtor(&a), "%d");
			cr_assert_eq(0, rnd_stack_set(s, i, &b), "%d");
			cr_assert_eq(0, rnd_stack_get(s, i, &a), "%d");
			cr_assert_eq(0, data_cmp(&a, &b), "%d");
		}
		cr_assert_eq(0, rnd_stack_destroy(s, data_dtor), "%d");
...
#define test(T, F1, F2, F3, V, M) do {                              \
		...
		s = rnd_stack_create(sizeof(T), 1000);              \
		cr_assert_neq(NULL, s);                                \
		cr_assert_eq(RND_EINDEX, F1(s, 0, a), "%d");       \
		cr_assert_eq(0, F2(s, (V)), "%d");                 \
		cr_assert_eq(RND_EINVAL, F1(NULL, 0, a), "%d");    \
		cr_assert_eq(RND_EINDEX, F1(s, 1, a), "%d");       \
		cr_assert_eq(0, F1(s, 0, a), "%d");                \
...
Test(stack, print)
{
	...
	{ /* Generic form */
		...
		cr_assert_eq(0, rnd_stack_push(s, &a), "%d");
		cr_assert_eq(0, rnd_stack_push(s, &b), "%d");
		cr_assert_eq(RND_EINVAL, rnd_stack_print(NULL), "%d");
		cr_assert_eq(0, rnd_stack_print(s), "%d");
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d");
	}
...
#define test(T, F1, F2, A, B)                                       \
	do {                                                        \
		...
		cr_assert_eq(0, F1(s, a), "%d");                   \
		cr_assert_eq(0, F1(s, b), "%d");                   \
		cr_assert_eq(RND_EINVAL, F2(NULL), "%d");          \
		cr_assert_eq(0, F2(s), "%d");                      \
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d"); \
		s = rnd_stack_create(sizeof(T) + 1, 30);            \
		cr_assert_eq(RND_EILLEGAL, F2(s), "%d");           \
		cr_assert_eq(0, rnd_stack_destroy(s, NULL), "%d"); \
	} while(0)
